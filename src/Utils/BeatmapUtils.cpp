...
namespace BeatmapUtils
{
    ...
    int getMaxScore(IBeatmapDataBasicInfo* beatmapDataBasicInfo)
    {
        int blockCount = beatmapDataBasicInfo->get_cuttableNotesCount();
    ...
    bool containsV3Stuff(IReadonlyBeatmapData* beatmapData)
    {
        ...
        {
            if (i->item->type == 0 && il2cpp_utils::try_cast<SliderData>(i->item).value_or(nullptr))
            {
                return true;
            }
        }
